...
func applyAdditionalFiltering(clientIP net.IP, clientID string, setts *filtering.Settings) {
	...
	log.Debug("finding settings for client with ip %s and clientid %q", clientIP, clientID)
	...
	log.Debug("using settings for client %s with ip %s and clientid %q", c.Name, clientIP, clientID)
